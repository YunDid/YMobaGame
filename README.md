...
## Ue Behaviour Tree / 行为树

> 用于 AI 行为的决策系统模型.

- Switch

  > 最简单的 AI 决策模型可以通过 case 分支来处理.
  >
  > `优缺点:`
  >
  > 1. 当决策累积时，存在大量的决策判断，代码量将十分庞大.
  > 2. 对于相似决策没有办法作优化.

- FSM / 有限状态机

  > 在将不同状态进行单独封装后，只需关注状态以及状态之间的切换即可.
  >
  > `优缺点:`
  >
  > 1. 对于相似判断仍没有办法作出优化.
...
- Behaviour Tree / 行为树
  ...
  > 每次作出 AI 决策时将从根部遍历行为树，父节点由子行为树的结果作出下一步决策.
  >
  > 行为树基本节点可分为两大类，一是控制节点(核心)，而是行为节点.
  >
  > `优缺点`
  >
  > 1. 易于拓展.
  > 2. 相似决策可属于同一父节点，极大地优化了决策消耗.
  >
  > `注意:`
  >
  > 1. Ue 下的行为树是事件驱动型的，此行为树将监听可影响决策的事件，并驱动行为树的决策.
  > 2. Ue 下的行为树叶子节点为行为节点(含具体执行任务)，其余节点为控制节点.
...
- UBlackboardData / 黑板数据

  > 黑板的原始数据，用于 BlackboardKeySelector  的初始化.

- BlackboardKeySelector / 键结构

  > 黑板下的某一栏目的键结构，可通过其获取对应键值对的键值.
  >
  > 该结构体下可通过 SelectedKeyType，SelectedKeyName 获取键值对的键名与键类型.

- UBlackboardKeyType / 键类型
...
- Selector Node 
  ...
  > 由左至右依次执行，若有一个分支执行成功，则停止执行，返回true.
  >
  > 当所有分支均执行失败后，则停止执行，返回false.

- Sequence Node 

  > 由左至右依次执行，若有一个分支执行失败，则停止执行，返回false.
  >
  > 当所有分支均执行成功后，则停止执行，返回true.
...
- Simple Parallel Node 
  ...
  > `注意:`
  >
  > 1. 次分支停止时机可设置主任务结束后立即停止或延缓直至此分支执行完毕停止.

#### BT - Decorator / 装饰器节点

> 条件判断节点，可以将于黑板键值的比较结果用于判断当前节点是否执行，是否终止.

#### BT - Service Node / 服务节点

> 可以连接至复合节点或任务节点，使其分支可以按自定义的频率执行.
>
> 常用于检查更新黑板值.
...
> 行为树的叶子节点，含具体的行为.
>
> 行为可分为两大类，一是瞬时任务，二是持续一段时间的任务.

### AI Controller / AI 控制器

> 使角色可以在不受人为控制的情况下，感知周围世界并作出响应.
>
> 具备移动导航和感知功能.
>
> `注意:`
>
> 1. 角色可设置自定义 AI Controller Class，用于自定义 AI 响应.